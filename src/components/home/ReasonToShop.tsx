    ...
    <section className="bg-gray-100 py-16">
      <div className="max-w-screen-xl mx-auto px-6">
        <div className={`relative ${bigShoulders.className}`}>
          <h1 className="text-8xl font-bold text-center tracking-wider bg-gradient-to-b mb-10 from-gray-300 via-gray-200 to-white bg-clip-text text-transparent">
            WHY CHOOSE US
          </h1>
          <p
            className={`uppercase text-5xl absolute font-black bottom-5 left-1/2 translate-x-[-50%] translate-y-1/2`}
          >
            Reasons to shop
          </p>
        </div>
        <div className="grid grid-cols-1 px-32 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {reasons.map((reason, index) => (
            <div
              key={index}
              className="bg-white p-8 rounded-lg text-center transition-transform duration-200 ease-linear transform hover:scale-105 cursor-pointer"
            >
